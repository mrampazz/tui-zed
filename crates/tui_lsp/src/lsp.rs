...
type PendingRespondTasks = Arc<Mutex<HashMap<RequestId, JoinHandle<()>>>>;
...
pub struct LanguageServer {
    ...
    io_tasks: Mutex<Option<(JoinHandle<Option<()>>, JoinHandle<Option<()>>)>>,
...
impl<'a> From<&'a str> for LanguageServerName {
    fn from(str: &'a str) -> LanguageServerName {
        LanguageServerName(Arc::from(str))
    }
}
...
impl LanguageServer {
        ...
        F: Fn(&NotificationOrRequest) -> bool + 'static + Send + Sync + Clone,
    {
        ...
        let stdout_input_task = executor.spawn({
        ...
        let stderr_input_task = stderr
            ...
            .unwrap_or_else(|| tokio::task::spawn(async { None }));
        let input_task = executor.spawn(async move {
            let (stdout, stderr) = futures::join!(stdout_input_task, stderr_input_task);
            stdout.ok().flatten().or(stderr.ok().flatten())
        });
        ...
        Stdout: AsyncRead + Unpin + Send + 'static,
    {
        ...
        input_handler.loop_handle.await.ok().transpose()?;
        Ok(())
    }
    ...
    pub fn initialize(
        ...
        executor.spawn(async move {
            ...
            if let Some(info) = response.server_info {
                self.version = info.version.map(|v| Arc::from(v.as_str()));
    ...
    pub fn shutdown(&self) -> Option<impl 'static + Send + Future<Output = Option<()>> + use<>> {
        ...
        let mut timer = Box::pin(tokio::time::sleep(SERVER_SHUTDOWN_TIMEOUT)).fuse();
        Some(async move {
    ...
    where
        ...
        F: 'static + FnMut(T::Params) -> Fut + Send,
    ...
    pub fn readable_version(&self) -> Option<Arc<str>> {
            ...
            "gopls" => {
                ...
                self.version
                    ...
                    .and_then(|obj| {
                        ...
                        let parsed: GoplsVersion = serde_json::from_str(&obj).ok()?;
                        Some(Arc::from(parsed.version.trim_start_matches("v")))
                    })
        ...
        T: request::Request,
    {
        Self::request_internal_with_timer::<T, _>(
            ...
            notification_serializers,
            executor,
            Self::request_timeout_future(request_timeout),
            params,
        )
    }
    ...
    pub fn request_timer(&self, timeout: Duration) -> impl Future<Output = String> {
        Self::request_timeout_future(timeout)
    }
